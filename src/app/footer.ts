...
@Component({
  ...
  template: `
    <div 
      (click)="cart.emit()" 
      class="flex flex-col items-center gap-2 transition-all duration-300 ease-in-out"
    >
      <button class=" bg-gradient-to-br from-emerald-500 to-emerald-600 w-14 h-14 rounded-full flex items-center justify-center">
    ...
    <div class="bg-black/10 h-14 grow rounded-full overflow-hidden">
      <textarea
        class="min-h-14 max-h-14 h-14 w-full bg-transparent outline-0 border-0 p-4"
    ...
    <div 
      ...
      class="flex flex-col items-center gap-2 transition-all duration-300 ease-in-out"
      [ngClass]="{
        '-ml-16 scale-0': focused(),
      }"
    >
      <button class="bg-gradient-to-bl from-red-500 to-red-700 w-14 h-14 rounded-full flex items-center justify-center">
    ...
    <div 
      (click)="send()" 
      class="flex flex-col items-center gap-2 transition-all duration-300 ease-in-out relative -right-2"
      [ngClass]="{
        '-ml-16 scale-0': !focused(),
      }"
    >
      <button class="bg-blue-500 w-14 h-14 rounded-full flex items-center justify-center">
